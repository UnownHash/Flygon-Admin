...
export const AreaForm = () => (
  <SimpleForm>
    ...
    <FormDataConsumer<Area>>
      {({ formData }) => {
        console.log(formData)
        const area = {
          geofence:
            formData?.geofence && Array.isArray(formData.geofence)
              ? formData.geofence
              : [],
          pokemon_mode: {
            route: Array.isArray(formData?.pokemon_mode?.route)
              ? formData.pokemon_mode.route
              : [],
          },
          fort_mode: {
            route: Array.isArray(formData?.fort_mode?.route)
              ? formData.fort_mode.route
              : [],
          },
          quest_mode: {
            route: Array.isArray(formData?.quest_mode?.route)
              ? formData.quest_mode.route
              : [],
          },
        }
